...
- **⚙️ 自定义配置**：支持为每个供应商自定义 API 地址和模型 ID，适配自建代理、私有化部署等高级场景；配置界面采用分层折叠设计，高级选项默认收起，保持清爽。
...
| **qwen3-tts** | 阿里云百炼 | 🇨🇳 🇺🇸 🇩🇪 🇫🇷 🇪🇸 <br>🇵🇹 🇮🇹 🇯🇵 🇰🇷 🇷🇺 | 48种音色，多语种支持 |
| **minimax-tts** | MiniMax | 🇨🇳 🇺🇸 | 32kHz 高采样率，WebSocket 流式 |
| **xiaomi-mimo-tts** | 小米 | 🇨🇳 🇺🇸 | OpenAI API 兼容 |

> **提示**：模型 ID 同时作为展示名称，强调模型本身。

## 🛠️ 技术栈

- **语言**: Kotlin 2.3.21
- **架构**: MVVM (Model-View-ViewModel) + Clean Architecture
- **UI**: Jetpack Compose (BOM 2026.05.00) + Material 3 Expressive
...
> **提示**：微软语音合成供应商（microsoft-tts）**无需任何配置**，开箱即用！

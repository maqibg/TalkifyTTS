...
> **供应商示例**：
> - **阿里云百炼（aliyunBailian）**：HTTP 流式合成供应商，配置类为 `AliyunBailianConfig`，供应商实现为 `AliyunBailianProvider`，音色定义在 `qwen3-tts-voices.xml`。
> - **微软 Azure 语音合成（azure）**：无需 API Key 的 TTS 服务，配置类为 `AzureConfig`，供应商实现为 `AzureProvider`，音色定义在 `microsoft-tts-voices.xml`。
> - **MiniMax 语音合成（miniMax）**：WebSocket 流式合成供应商，配置类为 `MiniMaxConfig`，供应商实现为 `MiniMaxProvider`，音色定义在 `minimax-voices.xml`，音频格式为 MP3，支持 `continuousSound` 连续发声开关。
> - **小米 MiMo 语音合成（xiaomi）**：HTTP 流式合成供应商，配置类为 `XiaomiConfig`，供应商实现为 `XiaomiProvider`，音色定义在 `xiaomi-mimo-voices.xml`。
...
### 8.1 版本号规范
- **versionName**: `Major.Minor.Patch` (当前 1.0.24)
- **versionCode**: 单调递增整数 (当前 26)
...
> **改版说明**：
> - **Provider 实现类去硬编码**：6 个供应商实现类的 `PROVIDER_ID`、`PROVIDER_NAME`、`DEFAULT_MODEL` 等硬编码常量已移除，统一改为从 `ProviderIds` 密封类引用，以 `ProviderIds` 为单一数据源，彻底消除两份数据不一致的问题。
> - **Provider 实现类重命名**：统一为供应商名命名（`Qwen3TtsProvider` → `AliyunBailianProvider`、`SeedTts2Provider` → `VolcengineProvider`、`TencentTtsProvider` → `TencentCloudProvider`、`MicrosoftTtsProvider` → `AzureProvider`、`MiniMaxTtsProvider` → `MiniMaxProvider`、`XiaoMiMimoTtsProvider` → `XiaomiProvider`）。
> - 统一类命名规范：Config 类由产品名命名改为供应商名命名（`Qwen3TtsConfig` → `AliyunBailianConfig`、`SeedTts2Config` → `VolcengineConfig`、`TencentTtsConfig` → `TencentCloudConfig`、`MicrosoftTtsConfig` → `AzureConfig`、`MiniMaxTtsConfig` → `MiniMaxConfig`、`XiaoMiMimoConfig` → `XiaomiConfig`），对应 Repository 类同步更名。
> - `ProviderIds` 由枚举改为密封类，每个 data object 包含三个属性：`providerId`（唯一标识）、`defaultModelId`（默认模型）、`provider`（展示名称），并通过 `toTtsProvider()` 扩展函数转换为 UI 数据模型。
> - 新增供应商自定义 API 地址和模型 ID 的配置能力。用户可在配置界面为每个供应商设置自定义的 API 端点和模型标识，未配置时自动使用供应商默认值。供应商展示界面会根据用户自定义的模型 ID 自适应更新"模型名称"。
> - **UI 优化**：将 API 地址和模型 ID 配置项折叠进"高级设置"面板，默认收起，减少配置界面的视觉压力。展开/折叠带有平滑的 Material 3 动画效果。详见 [6.3 配置编辑器 UI 规范](#63-配置编辑器-ui-规范) 章节。

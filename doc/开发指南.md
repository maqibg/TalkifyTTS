...
├── GlobalException.kt                    // 【异常处理】TalkifyExceptionHandler + TalkifyAppHolder 全局上下文
...
│   │   ├── ProviderIds.kt                // 供应商 ID 定义
│   │   ├── Qwen3TtsConfig.kt             // 通义千问 3 供应商配置
│   │   ├── SeedTts2Config.kt             // 豆包 Seed TTS 2 供应商配置
│   │   ├── TencentTtsConfig.kt           // 腾讯语音合成供应商配置
│   │   ├── MicrosoftTtsConfig.kt         // 微软语音合成供应商配置
│   │   ├── MiniMaxTtsConfig.kt           // MiniMax 语音合成供应商配置
│   │   ├── XiaoMiMimoConfig.kt           // 小米 MiMo 语音合成供应商配置
│   │   ├── TtsProviderRegistry.kt        // 供应商注册中心
...
- **`TtsProviderRegistry`**: 供应商注册中心，以 `ProviderIds` 密封类为单一数据源，管理所有可用供应商元信息。
...
1. **定义配置**：在 `domain/model` 下创建 `XxxConfig`，继承 `BaseProviderConfig`，包含 `apiUrl` 和 `modelId` 等 override 字段。
2. **实现供应商**：在 `service/provider/impl` 下创建 `XxxTtsProvider`，继承 `AbstractTtsProvider`，实现合成逻辑。
...
3. **实现仓储**：在 `infrastructure/provider/repo` 下创建对应的 Voice 和 Config 仓储实现，确保存取 `api_url` 和 `model_id` 字段。
4. **注册供应商**：
   - 在 `ProviderIds` 中添加供应商 ID 枚举。
   - 在 `TtsProviderRegistry` 中注册供应商信息。
   - 在 `TtsProviderFactory` 中注册供应商实例创建逻辑（如果使用工厂模式）。
5. **添加音色资源**：在 `res/xml` 下创建音色 XML 资源文件，通过 `VoiceXmlParser` 解析。
6. **编写测试**：在 `app/src/test/java` 下编写单元测试验证供应商功能。

> **供应商示例**：
> - **微软语音合成（microsoft-tts）**：无需 API Key 的 TTS 服务，配置类为 `MicrosoftTtsConfig`，供应商实现为 `MicrosoftTtsProvider`，音色定义在 `microsoft-tts-voices.xml`。
> - **MiniMax 语音合成（minimax-tts）**：WebSocket 流式合成供应商，配置类为 `MiniMaxTtsConfig`，供应商实现为 `MiniMaxTtsProvider`，音色定义在 `minimax-voices.xml`，音频格式为 MP3。
> - **小米 MiMo 语音合成（xiaomi-mimo-tts）**：HTTP 流式合成供应商，配置类为 `XiaoMiMimoConfig`，供应商实现为 `XiaoMiMimoTtsProvider`，音色定义在 `xiaomi-mimo-voices.xml`。

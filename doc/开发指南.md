...
2. **实现供应商**：在 `service/provider/impl` 下创建 `XxxTtsProvider`（如 `Qwen3TtsProvider`、`SeedTts2Provider`），继承 `AbstractTtsProvider`，实现合成逻辑。
...
> **供应商示例**：
> - **微软语音合成（microsoft-tts）**：无需 API Key 的 TTS 服务，配置类为 `AzureConfig`，供应商实现为 `MicrosoftTtsProvider`，音色定义在 `microsoft-tts-voices.xml`。
> - **MiniMax 语音合成（minimax-tts）**：WebSocket 流式合成供应商，配置类为 `MiniMaxConfig`，供应商实现为 `MiniMaxTtsProvider`，音色定义在 `minimax-voices.xml`，音频格式为 MP3，支持 `continuousSound` 连续发声开关。
> - **小米 MiMo 语音合成（xiaomi-mimo-tts）**：HTTP 流式合成供应商，配置类为 `XiaomiConfig`，供应商实现为 `XiaoMiMimoTtsProvider`，音色定义在 `xiaomi-mimo-voices.xml`。
